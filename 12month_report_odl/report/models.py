...
class TwelveMonthReport(models.AbstractModel):
    ...
    def get_xlsx(self, options,response):
     ...
     #    for line in lines:
     #        print ("lll",line)
     #        sheet.write(y_offset+1, 1,self._product_name(line),self.format_title_data)
     #        sheet.write(y_offset+1, 2,self._product_brand(line),self.format_title_data)
     #        sheet.write(y_offset+1, 3,self._product_code(line),self.format_title_data)
     #    workbook.close()
    	# output.seek(0)
